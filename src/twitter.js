...
export default async function twitterFeed(handle, token, startUpDate, handleDateMap) {
	const cookies = token;
    ...
    return page.$$('::-p-xpath(/html/body//article)')
        .then((articles) => Promise.all(
            ...
            articles.reverse().map(async (tweet) => {
                ...
                const lastDate = handleDateMap.has(handle) ? handleDateMap.get(handle) : startUpDate;
